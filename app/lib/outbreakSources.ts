...
//
// Each entry includes: a stable id, display name, feed URL, and a source
// label used in the UI. URLs verified May 2026 against each agency's RSS
// directory or API. If a URL changes, update here only — no other code
// changes needed.
//
// Sources rationale:
...
//     internationally significant outbreaks. WHO's HTML listing page is
//     now JS-rendered (server-side fetches return an empty page), so we
//     fetch directly from WHO's Sitefinity REST API instead. Returns JSON.
...
export type OutbreakSource = {
  ...
  url: string; // feed URL (RSS/Atom) or API endpoint
  format: "rss" | "atom" | "json-who";
...
export const OUTBREAK_SOURCES: OutbreakSource[] = [
  ...
  },
  {
    ...
    shortName: "WHO",
    // Sitefinity REST API. Returns JSON array of DON items, newest first.
    url: "https://www.who.int/api/news/diseaseoutbreaknews?$top=20&$orderby=PublicationDateAndTime%20desc",
    format: "json-who",
    region: "global",
  },
];
